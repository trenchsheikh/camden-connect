  ...
  return (
    <main className="mx-auto w-full max-w-3xl space-y-6 px-6 py-10">
      ...
      <form onSubmit={verifyCode} className="space-y-3 rounded-2xl border border-border-soft bg-surface p-6 shadow-sm">
        ...
        <p className="text-sm text-emerald-900/70">Demo valid code: {clubCode}</p>
      </form>

      {verified && (
        <form onSubmit={completeProfile} className="space-y-3 rounded-2xl border border-border-soft bg-surface p-6 shadow-sm">
          <h2 className="text-xl font-semibold">Complete your profile</h2>
          <input className="w-full rounded-lg border border-border-soft p-3" placeholder="Full name" required />
          <input className="w-full rounded-lg border border-border-soft p-3" placeholder="Age (17-24)" min={17} max={24} required type="number" />
          <select className="w-full rounded-lg border border-border-soft p-3" required>
            <option value="">Current situation</option>
            <option>Working</option>
            <option>Studying</option>
            <option>Looking for work</option>
          </select>
          <textarea className="w-full rounded-lg border border-border-soft p-3" placeholder="Your 3-year goal" required />
          <button type="submit" className="rounded-lg bg-accent px-4 py-2 font-medium text-white transition hover:bg-accent-strong">
            Save profile
          </button>
        </form>
